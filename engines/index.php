<?php
    ...
    $columns = array(
         "2" => array("name"=>"engine_id", "title"=>"Κωδικός", "cursor", "visible"),
         "3" => array("name"=>"serial_number", "title"=>"Σειριακός Αρ.", "cursor", "visible"),
         "4" => array("name"=>"engine_type", "title"=>"Τύπος", "cursor", "visible"),
         "5" => array("name"=>"engine_kind", "title"=>"Είδος ", "cursor", "visible"),
         "6" => array("name"=>"power", "title"=>"Δύναμη", "cursor"),
         "7" => array("name"=>"engine_power_type", "title"=>"Μονάδα", "cursor"),
         "8" => array("name"=>"engine_power", "title"=>"Ισχυς ", "cursor", "visible"),
         "9" => array("name"=>"engine_brand", "title"=>"Μάρκα", "cursor", "visible"),
        "10" => array("name"=>"engine_status", "title"=>"Κατάσταση", "cursor", "visible"),
        "11" => array("name"=>"count_boats", "title"=>"Σκάφη", "cursor", "visible" ),
        "12" => array("name"=>"count_owners", "title"=>"Ιδιοκτήτες", "cursor", "visible" ),
        "13" => array("name"=>"comments", "title"=>"Σχόλια", "cursor"),
    );
    ...
    $orderBy = $_SESSION["states"][$package]["orderBy"] ? : "serial_number";
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                if (data.status == 200)
                {
                    ...
                    $("#txtSerialNumber").val('<?php echo $advSearch["txtSerialNumber"] ?>')
                    $('#spEngineType').selectpicker('val', '<?php echo $advSearch["spEngineType"]?:0 ?>');
                    $('#spEngineKind').selectpicker('val', '<?php echo $advSearch["spEngineType"]?:0 ?>');
                    $("#txtPower").val('<?php echo $advSearch["txtPower"] ?>')
                    $('#spEnginePowerType').selectpicker('val', '<?php echo $advSearch["spEnginePowerType"]?:0 ?>');
                    $('#spEngineBrand').selectpicker('val', '<?php echo $advSearch["spEngineBrand"]?:0 ?>');
                    $('#spEngineStatus').selectpicker('val', '<?php echo $advSearch["spEngineStatus"]?:0 ?>');
            ...
            success: function(response){
                ...
                if (data.status == 200)
                {
                    ...
                    if (data.rows.length > 0)
                    {
                        $.each(data.rows, function(id, row)
                        {
                            var btnPrintInfos = '<td style="width: 80px"><button type="button" value="' + row.engine_id + '" class="btn btn-default btn-lg btn-block glyphicon glyphicon-print"></button></td>';
                            var $btnPrintInfos = $(btnPrintInfos).click(function() {
                                bootbox.prompt({
                                    ...
                                    callback : function(option) {
                                        ...
                                        {
                                            case "PrintInfos" :
                                                var data = {id:row.engine_id, print:option};
                            ...
                            var $btnEdit = $(btnEdit).click(function() {
                                window.location.href = "update/?id=" + row.engine_id;
                            });

                            <?php if (in_array($User["permission_id"], array(1, 2))) { ?>
                            var btnDelete = '<td style="width: 80px"><button type="button" value="' + row.engine_id + '" class="btn btn-default btn-lg btn-block glyphicon glyphicon-trash"></button></td>';
                            var $btnDelete = $(btnDelete).click(function() {
                                bootbox.dialog({
                                    message: 'Διαγραφή της Μηχανής "'+row.engine_id+' : '+row.serial_number+'";',
                                    title: "Διαγραφή",
                                    buttons: {
                                        success: {
                                            label: "Διαγραφή",
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                if (data.status == 200)
                {
                    ...
                    $('#spEngineType').append( '<option value="0">Τύπος</option>');
                    $('#spEngineType').append( '<option data-divider="true"></option>');
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                if (data.status == 200)
                {
                    ...
                    $('#spEngineKind').append( '<option value="0">Είδος</option>');
                    $('#spEngineKind').append( '<option data-divider="true"></option>');
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                if (data.status == 200)
                {
                    ...
                    $('#spEnginePowerType').append( '<option value="0">Μονάδα</option>');
                    $('#spEnginePowerType').append( '<option data-divider="true"></option>');
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                if (data.status == 200)
                {
                    ...
                    $('#spEngineBrand').append( '<option value="0">Μάρκα</option>');
                    $('#spEngineBrand').append( '<option data-divider="true"></option>');
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                if (data.status == 200)
                {
                    ...
                    $('#spEngineStatus').append( '<option value="0">Κατάσταση</option>');
                    $('#spEngineStatus').append( '<option data-divider="true"></option>');
...
////= Buttons ==========================================================================================================
    ...
    $("#btnAdvClear").click(function(event){
        $("#txtSerialNumber").val('');
        $('#spEngineType').selectpicker('val',0);
        $('#spEngineKind').selectpicker('val',0);
        $("#txtPower").val('');
        $('#spEnginePowerType').selectpicker('val',0);
        $('#spEngineBrand').selectpicker('val',0);
        $('#spEngineStatus').selectpicker('val',0);
        $("#txtComments").val('');
                        ...
                        <form class="form-horizontal col-sm-12" id="frm" role="form">
                            <fieldset>
                                ...
                                <div class="form-group">
                                    <div class="col-sm-4">
                                        <input type="text" class="form-control" name="txtSerialNumber" id="txtSerialNumber" placeholder="Σειριακός Αριθμός">
                                    </div>
                                    <div class="col-sm-4">
                                ...
                                <div class="form-group">
                                    <div class="col-sm-4">
                                        <input type="text" class="form-control" name="txtPower" id="txtPower" placeholder="Δύναμη">
                                    </div>
                                    <div class="col-sm-4">
                                        <div class="input-group">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-8">
                                        <input type="text" class="form-control" name="txtComments" id="txtComments" placeholder="Σχόλια">
                                    </div>
                                </div>
