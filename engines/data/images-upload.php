...
try
{
    ...
    else
    {
        ...
        $DestinationDirectory	= '../../media/engines/'; //specify upload directory ends with / (slash)
        ...
        $sql = "INSERT INTO engine_images SET
                    engine_id = ".$db->quote($id);
        //echo "<br><br>".$sql."<br><br>";

        $db->query( $sql );

        $engine_image_id = $db->lastInsertId();

        $NewImageName = $id.'-'.$engine_image_id.'.'.$ImageExt;

        $DestImageName = $DestinationDirectory.$NewImageName; // Image with destination directory

        $sql = "UPDATE engine_images SET
        ...
        $result["message"] = "Η εικόνα ανέβηκε".($AppVars["debug"] ? "  SQL >> ".$sql : "");
        $result["id"] = $id.":".$engine_image_id;
    }
}
catch (Exception $e)
{
