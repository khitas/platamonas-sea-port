<?php
    ...
    $pdf->MultiCell($Column01, 6, "Σειριακός Αριθμός", 1, 'C', 1, 0, 0, 0, true, 0);
    $pdf->MultiCell($Column00, 6, "Τύπος",             1, 'C', 1, 0, 0, 0, true, 0);
    $pdf->MultiCell($Column01, 6, "Είδος",             1, 'C', 1, 0, 0, 0, true, 0);
    $pdf->MultiCell($Column00, 6, "Δύναμη",            1, 'C', 1, 0, 0, 0, true, 0);
    $pdf->MultiCell($Column01, 6, "Μάρκα",             1, 'C', 1, 0, 0, 0, true, 0);
    $pdf->MultiCell($Column00, 6, "Κατάσταση",         1, 'C', 1, 1, 0, 0, true, 0);
    ...
    foreach( $data->rows as $row)
    {
        ...
        $pdf->MultiCell($Column00, 6, $row->engine_id,     1, 'L', 0, 0, 0, 0, true, 0);
        $pdf->MultiCell($Column01, 6, $row->serial_number, 1, 'L', 0, 0, 0, 0, true, 0);
        $pdf->MultiCell($Column00, 6, $row->engine_type,   1, 'L', 0, 0, 0, 0, true, 0);
        $pdf->MultiCell($Column01, 6, $row->engine_kind,   1, 'L', 0, 0, 0, 0, true, 0);
        $pdf->MultiCell($Column00, 6, $row->engine_power,  1, 'L', 0, 0, 0, 0, true, 0);
        $pdf->MultiCell($Column01, 6, $row->engine_brand,  1, 'L', 0, 0, 0, 0, true, 0);
        $pdf->MultiCell($Column00, 6, $row->engine_status, 1, 'L', 0, 1, 0, 0, true, 0);
    }
