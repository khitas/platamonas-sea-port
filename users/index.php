<?php
    ...
    $columns = array(
         "2" => array("name"=>"user_id", "title"=>"Κωδικός", "cursor", "visible"),
         "3" => array("name"=>"lastname", "title"=>"Επώνυμο", "cursor", "visible"),
         "4" => array("name"=>"firstname", "title"=>"Όνομα", "cursor", "visible"),
         "5" => array("name"=>"username", "title"=>"Όνομα Χρήστη", "cursor", "visible"),
         "6" => array("name"=>"permission", "title"=>"Δικαιώματα", "cursor", "visible"),
    );
...
//// Objects ===========================================================================================================
    ...
    $('#spPermission').selectpicker({size: 'auto', width:'100%', noneSelectedText : 'Δικαιώματα'});
            ...
            success: function(response){
                ...
                if (data.status == 200)
                {
                    ...
                    if (data.rows.length > 0)
                    {
                        $.each(data.rows, function(id, row)
                        {
                            var btnEdit = '<td style="width: 80px"><button type="button" value="' + row.user_id + '" class="btn btn-default btn-lg btn-block glyphicon glyphicon-<?php echo (in_array($User["permission_id"], array(1, 2)) ? "pencil" : "info-sign") ?>"></button></td>';
                            var $btnEdit = $(btnEdit).click(function() {
                                window.location.href = "update/?id=" + row.user_id;
                            });

                            <?php if (in_array($User["permission_id"], array(1, 2))) { ?>
                            var btnDelete = '<td style="width: 80px"><button type="button" value="' + row.user_id + '" class="btn btn-default btn-lg btn-block glyphicon glyphicon-trash"></button></td>';
                            var $btnDelete = $(btnDelete).click(function() {
                                bootbox.dialog({
                                    message: 'Διαγραφή του Χρήστη "'+row.user_id+' : '+row.fullname+'";',
                                    title: "Διαγραφή",
                                    buttons: {
                                        success: {
                                            label: "Διαγραφή",
                        ...
                        <form class="form-horizontal col-sm-12" id="frm" role="form">
                            <fieldset>
                                ...
                                <div class="form-group">
                                    <div class="col-sm-2"></div>
                                    <div class="col-sm-4">
                                        <input type="text" class="form-control" name="txtLastname" id="txtLastname" placeholder="Όνομα Χρήστη">
                                    </div>
                                    <div class="col-sm-4">
