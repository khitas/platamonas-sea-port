        ...
        $.ajax({
            ...
            success: function(response){
                ...
                if (data.status == 200)
                {
                    ...
                    $("#txtRePassword").prop('readonly', true);
                    $('#spPermission').prop('disabled',true);

                    <?php } ?>

                    <?php if (!in_array($User["permission_id"], array(1, 2))) { ?>
                    $("#btnPermission").prop('disabled',true);
                    <?php } ?>

                    $("#events-result").attr("class", "alert alert-info");
                    $("#events-result").text("<?php echo (in_array($User["permission_id"], array(1, 2)) ? "Ενημέρωση Χρήστη" : "Προβολή Χρήστη") ?>");
                }
                else
                {
