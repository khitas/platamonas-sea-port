...
try
{
    ...
    if ( $_POST["id"] ) {
        ...
        $where[] = "boat_types.boat_type_id = " . $db->quote($_POST["id"]);
    }

    if ( $_POST["boat_type_id"] ) {
        if (( ! ctype_digit($_POST["boat_type_id"]) ) || ($_POST["boat_type_id"] == 0) ) { throw new Exception("Ο Κωδικός είναι λάθος", 500);  }
        $where[] = "boat_types.boat_type_id = " . $db->quote($_POST["boat_type_id"]);
    }

    if ( ($_POST["searchColumn"] == "boat_type_id") && ($_POST["searchText"]) ) {
        if (( ! ctype_digit($_POST["searchText"]) ) || ($_POST["searchText"] == 0) ) { throw new Exception("Ο Κωδικός είναι λάθος", 500);  }
        $where[] = "boat_types.boat_type_id = " . $_POST["searchText"];
    }
    ...
    if ( ($_POST["searchColumn"] == "is_default") && ($_POST["searchText"]) ) {
        ...
        $where[] = "boat_types.is_default" . ($is_default ? " = " . $is_default : " is NULL");
    }
    ...
    if ( $_POST["txtName"] ) {
        $where[] = "boat_types.name like " . $db->quote("%".$_POST["txtName"]."%");
    }
    ...
    {
        case "id" : $sortby = "boat_types.boat_type_id"; break;
        case "boat_type_id" : $sortby = "boat_types.boat_type_id"; break;
        case "name" : $sortby = "boat_types.name"; break;
        case "is_default" : $sortby = "boat_types.is_default"; break;
        default : $sortby = "boat_types.name"; break;
    }
    ...
    $sql = "SELECT count(*) as total FROM boat_types ".$where;
