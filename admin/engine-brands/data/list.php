...
try
{
    ...
    if ( $_POST["id"] ) {
        ...
        $where[] = "engine_brands.engine_brand_id = " . $db->quote($_POST["id"]);
    }

    if ( $_POST["engine_brand_id"] ) {
        if (( ! ctype_digit($_POST["engine_brand_id"]) ) || ($_POST["engine_brand_id"] == 0) ) { throw new Exception("Ο Κωδικός είναι λάθος", 500);  }
        $where[] = "engine_brands.engine_brand_id = " . $db->quote($_POST["engine_brand_id"]);
    }

    if ( ($_POST["searchColumn"] == "engine_brand_id") && ($_POST["searchText"]) ) {
        if (( ! ctype_digit($_POST["searchText"]) ) || ($_POST["searchText"] == 0) ) { throw new Exception("Ο Κωδικός είναι λάθος", 500);  }
        $where[] = "engine_brands.engine_brand_id = " . $_POST["searchText"];
    }
    ...
    if ( ($_POST["searchColumn"] == "is_default") && ($_POST["searchText"]) ) {
        ...
        $where[] = "engine_brands.is_default" . ($is_default ? " = " . $is_default : " is NULL");
    }
    ...
    if ( $_POST["txtName"] ) {
        $where[] = "engine_brands.name like " . $db->quote("%".$_POST["txtName"]."%");
    }
    ...
    {
        case "id" : $sortby = "engine_brands.engine_brand_id"; break;
        case "engine_brand_id" : $sortby = "engine_brands.engine_brand_id"; break;
        case "name" : $sortby = "engine_brands.name"; break;
        case "is_default" : $sortby = "engine_brands.is_default"; break;
        default : $sortby = "engine_brands.name"; break;
    }
    ...
    $sql = "SELECT count(*) as total FROM engine_brands ".$where;
