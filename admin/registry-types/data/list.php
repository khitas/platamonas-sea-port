...
try
{
    ...
    if ( $_POST["id"] ) {
        ...
        $where[] = "registry_types.registry_type_id = " . $db->quote($_POST["id"]);
    }

    if ( $_POST["registry_type_id"] ) {
        if (( ! ctype_digit($_POST["registry_type_id"]) ) || ($_POST["registry_type_id"] == 0) ) { throw new Exception("Ο Κωδικός είναι λάθος", 500);  }
        $where[] = "registry_types.registry_type_id = " . $db->quote($_POST["registry_type_id"]);
    }

    if ( ($_POST["searchColumn"] == "registry_type_id") && ($_POST["searchText"]) ) {
        if (( ! ctype_digit($_POST["searchText"]) ) || ($_POST["searchText"] == 0) ) { throw new Exception("Ο Κωδικός είναι λάθος", 500);  }
        $where[] = "registry_types.registry_type_id = " . $_POST["searchText"];
    }
    ...
    if ( ($_POST["searchColumn"] == "is_default") && ($_POST["searchText"]) ) {
        ...
        $where[] = "registry_types.is_default" . ($is_default ? " = " . $is_default : " is NULL");
    }
    ...
    if ( $_POST["txtName"] ) {
        $where[] = "registry_types.name like " . $db->quote("%".$_POST["txtName"]."%");
    }
    ...
    {
        case "id" : $sortby = "registry_types.registry_type_id"; break;
        case "registry_type_id" : $sortby = "registry_types.registry_type_id"; break;
        case "name" : $sortby = "registry_types.name"; break;
        case "is_default" : $sortby = "registry_types.is_default"; break;
        default : $sortby = "registry_types.name"; break;
    }
    ...
    $sql = "SELECT count(*) as total FROM registry_types ".$where;
