...
try
{
    ...
    if ( $_POST["id"] ) {
        ...
        $where[] = "engine_power_types.engine_power_type_id = " . $db->quote($_POST["id"]);
    }

    if ( $_POST["engine_power_type_id"] ) {
        if (( ! ctype_digit($_POST["engine_power_type_id"]) ) || ($_POST["engine_power_type_id"] == 0) ) { throw new Exception("Ο Κωδικός είναι λάθος", 500);  }
        $where[] = "engine_power_types.engine_power_type_id = " . $db->quote($_POST["engine_power_type_id"]);
    }

    if ( ($_POST["searchColumn"] == "engine_power_type_id") && ($_POST["searchText"]) ) {
        if (( ! ctype_digit($_POST["searchText"]) ) || ($_POST["searchText"] == 0) ) { throw new Exception("Ο Κωδικός είναι λάθος", 500);  }
        $where[] = "engine_power_types.engine_power_type_id = " . $_POST["searchText"];
    }
    ...
    if ( ($_POST["searchColumn"] == "is_default") && ($_POST["searchText"]) ) {
        ...
        $where[] = "engine_power_types.is_default" . ($is_default ? " = " . $is_default : " is NULL");
    }
    ...
    if ( $_POST["txtName"] ) {
        $where[] = "engine_power_types.name like " . $db->quote("%".$_POST["txtName"]."%");
    }
    ...
    {
        case "id" : $sortby = "engine_power_types.engine_power_type_id"; break;
        case "engine_power_type_id" : $sortby = "engine_power_types.engine_power_type_id"; break;
        case "name" : $sortby = "engine_power_types.name"; break;
        case "is_default" : $sortby = "engine_power_types.is_default"; break;
        default : $sortby = "engine_power_types.name"; break;
    }
    ...
    $sql = "SELECT count(*) as total FROM engine_power_types ".$where;
    ...
    $sql = "SELECT
              engine_power_type_id,
              name,
              is_default
            FROM engine_power_types "
