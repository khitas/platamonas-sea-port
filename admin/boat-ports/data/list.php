...
try
{
    ...
    if ( $_POST["id"] ) {
        ...
        $where[] = "boat_ports.boat_port_id = " . $db->quote($_POST["id"]);
    }

    if ( $_POST["boat_port_id"] ) {
        if (( ! ctype_digit($_POST["boat_port_id"]) ) || ($_POST["boat_port_id"] == 0) ) { throw new Exception("Ο Κωδικός είναι λάθος", 500);  }
        $where[] = "boat_ports.boat_port_id = " . $db->quote($_POST["boat_port_id"]);
    }

    if ( ($_POST["searchColumn"] == "boat_port_id") && ($_POST["searchText"]) ) {
        if (( ! ctype_digit($_POST["searchText"]) ) || ($_POST["searchText"] == 0) ) { throw new Exception("Ο Κωδικός είναι λάθος", 500);  }
        $where[] = "boat_ports.boat_port_id = " . $_POST["searchText"];
    }
    ...
    if ( ($_POST["searchColumn"] == "is_default") && ($_POST["searchText"]) ) {
        ...
        $where[] = "boat_ports.is_default" . ($is_default ? " = " . $is_default : " is NULL");
    }
    ...
    if ( $_POST["txtName"] ) {
        $where[] = "boat_ports.name like " . $db->quote("%".$_POST["txtName"]."%");
    }
    ...
    {
        case "id" : $sortby = "boat_ports.boat_port_id"; break;
        case "boat_port_id" : $sortby = "boat_ports.boat_port_id"; break;
        case "name" : $sortby = "boat_ports.name"; break;
        case "is_default" : $sortby = "boat_ports.is_default"; break;
        default : $sortby = "boat_ports.name"; break;
    }
    ...
    $sql = "SELECT count(*) as total FROM boat_ports ".$where;
