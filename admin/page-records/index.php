<?php
    ...
    $columns = array(
         "2" => array("name"=>"page_record_id", "title"=>"Κωδικός", "cursor", "visible"),
            ...
            success: function(response){
                ...
                if (data.status == 200)
                {
                    ...
                    if (data.rows.length > 0)
                    {
                        $.each(data.rows, function(id, row)
                        {
                            var btnEdit = '<td style="width: 80px"><button type="button" value="' + row.page_record_id + '" class="btn btn-default btn-lg btn-block glyphicon glyphicon-<?php echo (in_array($User["permission_id"], array(1, 2)) ? "pencil" : "info-sign") ?>"></button></td>';
                            var $btnEdit = $(btnEdit).click(function() {
                                window.location.href = "update/?id=" + row.page_record_id;
                            });

                            <?php if (in_array($User["permission_id"], array(1, 2))) { ?>
                            var btnDelete = '<td style="width: 80px"><button type="button" value="' + row.page_record_id + '" class="btn btn-default btn-lg btn-block glyphicon glyphicon-trash"></button></td>';
                            var $btnDelete = $(btnDelete).click(function() {
                                bootbox.dialog({
                                    message: 'Διαγραφή του "'+row.page_record_id+' : '+row.name+'";',
                                    title: "Διαγραφή",
                                    buttons: {
                                        success: {
                                            label: "Διαγραφή",
