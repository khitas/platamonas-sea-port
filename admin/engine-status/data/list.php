...
try
{
    ...
    if ( $_POST["id"] ) {
        ...
        $where[] = "engine_status.engine_status_id = " . $db->quote($_POST["id"]);
    }

    if ( $_POST["engine_status_id"] ) {
        if (( ! ctype_digit($_POST["engine_status_id"]) ) || ($_POST["engine_status_id"] == 0) ) { throw new Exception("Ο Κωδικός είναι λάθος", 500);  }
        $where[] = "engine_status.engine_status_id = " . $db->quote($_POST["engine_status_id"]);
    }

    if ( ($_POST["searchColumn"] == "engine_status_id") && ($_POST["searchText"]) ) {
        if (( ! ctype_digit($_POST["searchText"]) ) || ($_POST["searchText"] == 0) ) { throw new Exception("Ο Κωδικός είναι λάθος", 500);  }
        $where[] = "engine_status.engine_status_id = " . $_POST["searchText"];
    }
    ...
    if ( ($_POST["searchColumn"] == "is_default") && ($_POST["searchText"]) ) {
        ...
        $where[] = "engine_status.is_default" . ($is_default ? " = " . $is_default : " is NULL");
    }
    ...
    if ( $_POST["txtName"] ) {
        $where[] = "engine_status.name like " . $db->quote("%".$_POST["txtName"]."%");
    }
    ...
    {
        case "id" : $sortby = "engine_status.engine_status_id"; break;
        case "engine_status_id" : $sortby = "engine_status.engine_status_id"; break;
        case "name" : $sortby = "engine_status.name"; break;
        case "is_default" : $sortby = "engine_status.is_default"; break;
        default : $sortby = "engine_status.name"; break;
    }
    ...
    $sql = "SELECT count(*) as total FROM engine_status ".$where;
