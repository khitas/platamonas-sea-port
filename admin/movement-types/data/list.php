...
try
{
    ...
    if ( $_POST["id"] ) {
        ...
        $where[] = "movement_types.movement_type_id = " . $db->quote($_POST["id"]);
    }

    if ( $_POST["movement_type_id"] ) {
        if (( ! ctype_digit($_POST["movement_type_id"]) ) || ($_POST["movement_type_id"] == 0) ) { throw new Exception("Ο Κωδικός είναι λάθος", 500);  }
        $where[] = "movement_types.movement_type_id = " . $db->quote($_POST["movement_type_id"]);
    }

    if ( ($_POST["searchColumn"] == "movement_type_id") && ($_POST["searchText"]) ) {
        if (( ! ctype_digit($_POST["searchText"]) ) || ($_POST["searchText"] == 0) ) { throw new Exception("Ο Κωδικός είναι λάθος", 500);  }
        $where[] = "movement_types.movement_type_id = " . $_POST["searchText"];
    }
    ...
    if ( ($_POST["searchColumn"] == "is_default") && ($_POST["searchText"]) ) {
        ...
        $where[] = "movement_types.is_default" . ($is_default ? " = " . $is_default : " is NULL");
    }
    ...
    if ( $_POST["txtName"] ) {
        $where[] = "movement_types.name like " . $db->quote("%".$_POST["txtName"]."%");
    }
    ...
    {
        case "id" : $sortby = "movement_types.movement_type_id"; break;
        case "movement_type_id" : $sortby = "movement_types.movement_type_id"; break;
        case "name" : $sortby = "movement_types.name"; break;
        case "is_default" : $sortby = "movement_types.is_default"; break;
        default : $sortby = "movement_types.name"; break;
    }
    ...
    $sql = "SELECT count(*) as total FROM movement_types ".$where;
