...
try
{
    ...
    if ( $_POST["id"] ) {
        ...
        $where[] = "engine_kinds.engine_kind_id = " . $db->quote($_POST["id"]);
    }

    if ( $_POST["engine_kind_id"] ) {
        if (( ! ctype_digit($_POST["engine_kind_id"]) ) || ($_POST["engine_kind_id"] == 0) ) { throw new Exception("Ο Κωδικός είναι λάθος", 500);  }
        $where[] = "engine_kinds.engine_kind_id = " . $db->quote($_POST["engine_kind_id"]);
    }

    if ( ($_POST["searchColumn"] == "engine_kind_id") && ($_POST["searchText"]) ) {
        if (( ! ctype_digit($_POST["searchText"]) ) || ($_POST["searchText"] == 0) ) { throw new Exception("Ο Κωδικός είναι λάθος", 500);  }
        $where[] = "engine_kinds.engine_kind_id = " . $_POST["searchText"];
    }
    ...
    if ( ($_POST["searchColumn"] == "is_default") && ($_POST["searchText"]) ) {
        ...
        $where[] = "engine_kinds.is_default" . ($is_default ? " = " . $is_default : " is NULL");
    }
    ...
    if ( $_POST["txtName"] ) {
        $where[] = "engine_kinds.name like " . $db->quote("%".$_POST["txtName"]."%");
    }
    ...
    {
        case "id" : $sortby = "engine_kinds.engine_kind_id"; break;
        case "engine_kind_id" : $sortby = "engine_kinds.engine_kind_id"; break;
        case "name" : $sortby = "engine_kinds.name"; break;
        case "is_default" : $sortby = "engine_kinds.is_default"; break;
        default : $sortby = "engine_kinds.name"; break;
    }
    ...
    $sql = "SELECT count(*) as total FROM engine_kinds ".$where;
