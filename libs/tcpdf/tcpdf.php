  ...
  */
  class TCPDF {
    ...
    */
     ...
     * @access public
     */
    public function setHeaderData($ln='', $lw=0, $ht='', $hs='', $ln2='', $lw2=0, $ht2='', $hs2='') {
      $this->header_logo = $ln;
      $this->header_logo_width = $lw;
      $this->header_title = $ht;
      $this->header_string = $hs;
      $this->header_logo2 = $ln2;
      $this->header_logo_width2 = $lw2;
      $this->header_title2 = $ht2;
    ...
    public function getHeaderData() {
      ...
      $ret['logo_width2'] = $this->header_logo_width2;
      $ret['title2'] = $this->header_title2;
      $ret['string2'] = $this->header_string2;
      return $ret;
    }
    ...
    public function MyHeader() {
      ...
      if (($headerdata['logo2']) AND ($headerdata['logo2'] != K_BLANK_IMAGE)) {
        $this->Image(K_PATH_IMAGES.$headerdata['logo2'], $this->GetX()+150, $this->getHeaderMargin(), $headerdata['logo_width2']);
      ...
      if (($headerdata['logo']) AND ($headerdata['logo'] != K_BLANK_IMAGE)) {
        ...
        $sql = "SELECT option_value FROM options WHERE option_name = 'report_header'";
        $stmt = $db->query( $sql );
        $row = $stmt->fetch(PDO::FETCH_ASSOC);


      $this->SetFont('freesans', '', 8);
      $this->MultiCell(0, 0, addslashes($row["option_value"]), 0, 'L', 0, 1, $headerdata['logo_width']+17, '', true, 0, false);
      //$this->MultiCell(0, 0, "Διεύθυνση Πληροφορικής & Τεχνικών Υπηρεσιών\nΚέντρο Διαχείρισης Δικτύου (NOC) - ΤΕΙ Αθήνας\nΑρωγή χρηστών (Helpdesk)", 0, 'L', 0, 0, 0, 30, true, 0, false);
      //$this->MultiCell(0, 0, "Διεύθυνση : Αγ. Σπυρίδωνα 28, ΑΙΓΑΛΕΩ 12210\nΤηλέφωνο : 2105385149, ή 5149 (εσωτερ.)\nΗλεκτρον. διεύθυνση : helpdesk@teiath.gr", 0, 'R', 0, 1, 0, 30, true, 0, false);

      $this->SetLineStyle(array('width' => 0.5 / $this->getScaleFactor(), 'cap' => 'butt', 'join' => 'miter', 'dash' => 0, 'color' => array(0, 0, 0)));
    ...
    public function MyFooter() {        
      ...
      $line_width = 0.5 / $this->getScaleFactor();
      $this->SetLineStyle(array('width' => $line_width, 'cap' => 'butt', 'join' => 'miter', 'dash' => 0, 'color' => array(0, 0, 0)));

        $this->SetFont('freesans', '', 9);

      if (empty($this->pagegroups)) {
        $pagenumtxt = "Σελίδα".' '.$this->getAliasNumPage().' / '.$this->getAliasNbPages();
      } else {
        $pagenumtxt = "Σελίδα".' '.$this->getPageNumGroupAlias().' / '.$this->getPageGroupAlias();
      }
      $this->SetY($cur_y);
      //Print page number
      if ($this->getRTL()) {
    ...
    protected function setFooter() {
      ...
      if ($this->print_footer) {
        ...
        $this->MultiCell(0, 0, $this->FooterMessage, 0, 'L', 0, 1, 0, -20, true, 0);
        $this->SetFont($this->footer_font[0], $this->footer_font[1], $this->footer_font[2]);
        if ( $this->ShowPageNumbers ) $this->MyFooter();
        //restore position
        if ($this->rtl) {
          $this->SetXY($this->original_rMargin, $this->tMargin);
        } else {
