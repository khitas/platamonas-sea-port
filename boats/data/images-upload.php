...
try
{
    ...
    else
    {
        ...
        $DestinationDirectory	= '../../media/boats/'; //specify upload directory ends with / (slash)
        ...
        $sql = "INSERT INTO boat_images SET
                    boat_id = ".$db->quote($id);
        //echo "<br><br>".$sql."<br><br>";

        $db->query( $sql );

        $boat_image_id = $db->lastInsertId();

        $NewImageName = $id.'-'.$boat_image_id.'.'.$ImageExt;

        $DestImageName = $DestinationDirectory.$NewImageName; // Image with destination directory

        $sql = "UPDATE boat_images SET
